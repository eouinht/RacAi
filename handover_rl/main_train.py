...
from train import PPOTrainConfig, train_one_env


HO_MARGIN_DB = 0.0


def choose_best_ru_by_sinr(
    ue_state: dict,
    rus_state: dict,
    ho_margin_db: float = HO_MARGIN_DB,
) -> int:
    ...
    best_ru = serving_ru
    best_score = current_sinr if current_sinr is not None else float("-inf")
    ...
    raw_best_sinr = air_metrics.get(best_ru, {}).get("sinr_db")
    if raw_best_sinr is not None and raw_best_sinr >= current_sinr + ho_margin_db:
        return best_ru
...
def print_action_summary(state: dict, actions: dict[int, UEAction]) -> None:
    print("Actions:")
...
def run_rule_based_debug(env: TraceDrivenHandoverEnv) -> None:
